...
abstract class GroovyBase extends UrlCommandCell
{
    ...
    protected Class compileAlt(Map<String, Object> ctx)
    {
        NCube cube = getNCube(ctx)
        String url = getUrl()
        boolean isUrlUsed = StringUtilities.hasContent(url)
        ...
        synchronized (GroovyBase.class)
        {
            ...
            File srcFile = new File(tempdir, 'N_' + cmdHash + '.groovy')
            srcFile.write(groovySource)

            gcLoader.addURL(srcFile.toURI().toURL())
            GroovyScriptEngine engine = new GroovyScriptEngine([srcFile.toURI().toURL()] as URL[])
            Class clazz = engine.getGroovyClassLoader().parseClass(srcFile)
//            println 'Engine ClassLoader URLs:'
//            println engine.groovyClassLoader.getURLs()
//            println '-------'
//            println 'NCUBE URLs:'
//            println gcLoader.getURLs()
//            println '-------'
            compiledMap[cmdHash] = clazz
            srcFile.delete()
            return clazz
