...
//            }
     ...
     */
    LongHashSet ensureFullCoordinate(Collection<Long> coordinate)
    {
        if (coordinate == null)
        {
    ...
    private static void addCoordinateToColumnEntry(Map<String, CellInfo> coord, Column column, Axis axis)
    {
        if (axis.getType() != AxisType.RULE)
        {
            coord[axis.getName()] = new CellInfo(column.getValueThatMatches())
        }
    }
    ...
    public Map<String, T> getDisplayCoordinateFromIds(Set<Long> idCoord)
    {
        ...
        for (Long colId : idCoord)
        {
            ...
            if (name != null)
            {
                properCoord[axis.getName()] = ("(" + name.toString() + "): " + value) as T
            }
            else
            {
                properCoord[axis.getName()] = value as T
            }
        }
        return properCoord
    }
